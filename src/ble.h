...
void initBle() {
	...
	for (auto& serviceConfig : server.services) {
		// FIX: Explizit auf BLEUUID casten
		serviceConfig.bleService =
		server.bleServer->createService(BLEUUID(serviceConfig.uuid.c_str()), 4 * serviceConfig.characteristics.size());
		...
		for (auto& characteristicConfig : serviceConfig.characteristics) {
			...
			serviceConfig.bleService->createCharacteristic(characteristicConfig.uuid.c_str(), property);
